    ...
    <main id="js-page-content" role="main" class="page-content">
        ...
        <div class="subheader">
            @component('layouts._page_heading', [
                ...
                'sup' => 'ADDON',
                'pagedescription' => 'Generate your unique stack Icon',
            ])
            @endcomponent
        </div>
        <div class="container">
            <div id="panel-1" class="panel">
                <div class="panel-hdr">
                    ...
                    <div class="panel-toolbar mr-2">
                        <select id="js-select-layers" class="custom-select form-control custom-select-sm"
                            style="width:8rem">
                    ...
                    <div class="panel-content">
                        <div class="row">
                            <div class="col-12 col-xl-auto">
                                <div class="d-flex align-items-center justify-content-center position-relative w-100 mb-g mb-xl-0"
                                    style="font-size: 250px; min-width: 301px; height: 351px; background: url(img/backgrounds/bg-5.png); border-left:0; border-top:0; ">
                                    <h6
...
@section('pages-script')
    <script>
        var fa_icon,
            ...
            ni_base,
            ng_textColors,
            fa_icon_URL = "{{ asset('') }}assets/media/data/fa-icon-list.json",
            fa_brand_URL = "{{ asset('') }}assets/media/data/fa-brand-list.json",
            ni_icon_URL = "{{ asset('') }}assets/media/data/ng-icon-list.json",
            ni_base_URL = "{{ asset('') }}assets/media/data/ng-icon-base.json",
            ng_textColors_URL = "{{ asset('') }}assets/media/data/ng-text-colors.json",
        ...
        $.when(
            ...
            if (fa_icon && fa_brand && ni_icon && ni_base && ng_textColors) {
                ...
                jQuery.each(iconSize, function(index, item) {
                    formatIconSize.push($('<option></option>').attr("value", item).text("size-" + item
                        .slice(11)))
                });
                //formatTextColors
                jQuery.each(ng_textColors, function(index, item) {
                    formatTextColors.push($('<option></option>').attr("value", item).addClass("bg" + item
                        .slice(5)).text(item.slice(6)))
                });
                //format_fa_icon
                jQuery.each(fa_icon, function(index, item) {
                    format_fa_icon.push('<option value="fa' + item + '" data-icon-class="' + item + '">' +
                        item.slice(1) + '</option>')
                });
                //fa_brand
                jQuery.each(fa_brand, function(index, item) {
                    format_fa_brand.push('<option value="fa' + item + '" data-icon-class="' + item + '">' +
                        item.slice(1) + '</option>')
                });
                //format_ni_icon
                jQuery.each(ni_icon, function(index, item) {
                    format_ni_icon.push('<option value="ni' + item + '" data-icon-class="' + item + '">' +
                        item.slice(1) + '</option>')
                });
                //format_ni_base
                jQuery.each(ni_base, function(index, item) {
                    format_ni_base.push('<option value="' + item + '" data-icon-class="' + item + '">' +
                        item + '</option>')
                });
                ...
                $(document).on('change', '.js-icon-prefix', function() {
                    ...
                    if (selectValue == 'fal' || selectValue == 'fas' || selectValue == 'far') {
                        $(this).closest('[data-layer-target]').find('.js-icon-class').empty().append(
                            format_fa_icon);
                ...
                var runIconGenerator = function(id) {
                    ...
                    $('#icon-construct').find('[data-icon-select="' + id + '"]').removeClass().addClass(
                        iconPrefix + " " + iconClass + " " + iconSize + " " + "opacity-" + iconOpacity +
                        " " + iconColor + " " + iconAnimate + " " + iconRotate);

                    uID.find('select.js-icon-color').removeClassPrefix('bg-').addClass('bg' + iconColor.slice(
                        5));
                };
                ...
                var changeOrder = function() {
                    ...
                    $('#construct-wrap li').each(function() {
                        ...
                        bundleNewElms.push('<i data-icon-select="' + uID + '" class="' + iconPrefix +
                            " " + iconClass + " " + iconSize + " " + "opacity-" + iconOpacity +
                            " " + iconColor + " " + iconAnimate + " " + iconRotate + '"></i>')
        ...
        var copyIcon = function() {
            $('#js-icon-class').text("<div class='icon-stack'>\n" + $('#icon-construct').html().replace(
                /data-icon-select="[^]"/g, ' ').replace(/\s\s+/g, ' ') + "\n</div>");
